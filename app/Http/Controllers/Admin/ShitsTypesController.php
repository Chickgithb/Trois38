...
use App\Http\Requests\Shifts_typeRequest;
use PhpParser\Node\Expr\AssignOp\ShiftLeft;

class ShitsTypesController extends Controller
{
    ...
    public function store(Shifts_typeRequest $request)
    {
        try {
            ...
            DB::commit();
            return redirect()->route('ShitsTypes.index')->with(['success' => 'تم تسجيل البيانات بنجاح']);
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public function edit($id){
        ...
        if(empty($data)){
            return redirect()->route('ShitsTypes.index')->with(['error'=>'عفوا لا يمكن الوصول الى البيانات المطلوبة']);
        }
    ...
    public function update($id, Shifts_typeRequest $request){
        ...
        try{
            ...
            $checkExsitsData = Shifts_type::select("id")->where('type','=',$request->type)->where('from_time','=',$request->from_time)->where('to_time','=',$request->to_time)->where('total_hour','=',$request->total_hour)->where('id','!=',$request->$id)->first();

            if(!empty($checkExsitsData)){
                return redirect()->route('ShitsTypes.index')->with(['error'=>'عفوا هذه البيانات مسجلة من قبل']);
            ...
            DB::commit();
            return redirect()->route('ShitsTypes.index')->with(['error'=>'ثم تعديل البيانات بنجاح']);
