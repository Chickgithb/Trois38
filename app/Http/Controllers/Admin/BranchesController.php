...
use App\Http\Requests\Branches_Request;
...
class BranchesController extends Controller
{
    ...
    public function store(Branches_Request $request){
        try{
            $com_code = auth()->user()->com_code;
            ...
            DB::commit();
            return redirect()->route('branches.index')->with(['success'=>'ثم ادخال البيانات بنجاح']);

        }catch(\Exception $ex){
            DB::rollBack();
            return redirect()->back()->with(['error'=>'عفوا هناك خطأ'.$ex->getMessage()])->withInput();
